...
contract WandProtocol is IWandProtocol, Ownable, ReentrancyGuard {
  ...
  constructor() Ownable() {
    // _settings = address(new ProtocolSettings(address(this), _msgSender()));
    // _usbToken = address(new USB(address(this), "USB Token", "USB"));
  }
  ...
  //   _settings = address(new ProtocolSettings(address(this), _msgSender()));
  //   _usbToken = address(new USB(address(this), "USB Token", "USB"));
  //   _assetPoolFactory = address(new AssetPoolFactory(address(this), _usbToken));
  //   _interestPoolFactory = address(new InterestPoolFactory(address(this)));
  ...
  function addAssetPool(
    ...
    IAssetPoolFactory(_assetPoolFactory).addAssetPool(assetToken, assetPriceFeed, xTokenName, xTokenSymbol, Y, AART, AARS, AARC);

    // Add newly created X token to all interest pools
    address xToken = IAssetPoolFactory(_assetPoolFactory).getAssetPoolXToken(assetToken);
